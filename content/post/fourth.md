...
我使用 PowerShell 执行 Hugo 命令，使用 VS Code 编辑博客文件，最后再通过 GitHub 保存和发布网站。

简单来说：

- PowerShell：用来执行命令，例如创建博客、启动网站；
- VS Code：用来编辑文章和修改配置文件；
- GitHub：用来保存代码，并通过 GitHub Pages 发布网站。
...
虽然 Hugo 本身并不复杂，但对于第一次搭建博客的人来说，真正困难的地方并不是输入几条命令，而是不知道每一步应该在哪里完成。

整个过程中，我需要同时使用多个工具：

- PowerShell：执行命令，创建项目和运行 Hugo；
- VS Code：编辑文章、修改配置文件；
- GitHub：保存博客文件，管理代码；
- GitHub Pages：发布网站。

刚开始接触这些工具时，它们之间的关系并不直观。

例如：

文件应该在哪里创建？
命令应该在 PowerShell 还是 VS Code 中输入？
修改完成后，如何同步到 GitHub？
为什么 GitHub 已经有文件，但网站没有更新？
部署失败后，应该在哪里查看问题？

这些问题在第一次搭建时都会遇到。

其中，最容易让人困惑的是 GitHub 的使用。GitHub 不只是一个存放代码的网站，它还包含仓库管理、权限设置、自动部署等功能。第一次接触时，很多页面和按钮的位置都需要慢慢熟悉。

例如，在使用 GitHub Pages 部署博客时，需要配置 Actions 自动部署流程。如果权限设置错误，或者部署方式选择不正确，网站就无法正常更新。

后来在不断尝试和排查问题的过程中，我逐渐理解了整个博客搭建流程：

本地电脑负责编写和修改博客；
Git 负责记录项目变化；
GitHub 负责保存代码；
GitHub Actions 负责自动构建网站；
GitHub Pages 负责将网站展示给其他人。

搭建博客的过程并不只是学习几个命令，更重要的是理解这些工具之间是如何协同工作的。
